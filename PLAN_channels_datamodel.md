...
## 4. `visibleUids` 유지 — Cloud Function이 아니라 원자적 배치로 (2026-08-24 구현 시 변경)

비공개 채널의 멤버가 바뀌면 그 채널에 속한 **모든 글**의 `visibleUids`를 갱신해야 한다.
설계 단계에서는 Cloud Function 트리거를 제안했으나, **구현하면서 클라이언트 원자적 배치로
바꿨다**(`channelActions.js`의 `updateChannelAndPosts`).

바꾼 이유:
- **Firestore `writeBatch`는 전부 되거나 전부 안 된다.** 채널 문서 수정과 글 수정을 한
  배치에 넣으면 "일부만 반영" 자체가 불가능하다 — 원래 CF를 제안한 근거가 사라진다
- 오히려 **CF 트리거가 더 약하다.** 트리거는 결과적 일관성이라 채널이 바뀐 뒤 글이 따라오기
  전까지 어긋나는 창이 생기고, 그 창이 곧 열람 권한이다
- 배포 표면이 줄어든다(functions 배포 불필요)

**한계**: 배치 상한이 500이라 글이 499건을 넘는 채널은 한 번에 못 맞춘다. 쪼개는 순간
원자성이 깨지므로 그때는 CF가 낫다. 지금 규모(학교 전체 글 4건)에서는 걸릴 일이 없다.

**부수 제약**: 새 규칙에서는 `where('channelId','==',id)`로 글을 조회할 수 없다(규칙 조건과
맞물리지 않아 쿼리가 거부된다). 그래서 갱신할 글 목록은 화면이 이미 들고 있는 것
(`useChannels`가 채널별로 나눠준 `channel.posts`)을 넘겨서 쓴다.

### 4.1 작성자의 `visibleUids` 위조는 막지 않기로 했다 (§3.5 재검토)

§3.5에서 "작성자가 `visibility`·`visibleUids`를 못 바꾸게 묶자"고 했으나, 구현하면서
**하지 않기로 판단**했다.

- 글을 고치려면 규칙상 **작성자이거나 학교 관리자**여야 한다
- 비공개 글의 작성자는 이미 그 채널의 참여자이고 내용을 갖고 있다. 즉 위조로 얻는 것이
  "자기가 쓴 글을 밖에 보이게 하는 것"뿐인데, 이는 내용을 복사해 붙이는 것과 다르지 않다
- 남의 비공개 글은 **읽을 수조차 없어** 문서 ID를 알 수 없다. 위조 대상이 되지 못한다

대신 규칙으로 막으려면 쓰기마다 채널 문서를 `get()`해 "이 값이 채널 상태와 맞는가"를 봐야
하는데, 규칙이 복잡해지고 채널이 지워진 글은 수정 자체가 막히는 새 실패 모드가 생긴다.
얻는 것에 비해 비싸다. **다만 이건 "안전하다"가 아니라 "이 위협 모델에서는 값이 안 나온다"는
판단이므로, 외부 사용자가 들어오는 구조가 되면 다시 봐야 한다.**
...
### P2b 구현 기록 (2026-08-24) — 설계에서 두 가지가 더 붙었다

**1. 정렬 여부를 규칙에서도 검사한다.** 위 초안은 "클라이언트가 정렬해 넣는다"를 전제로
뒀는데, 전제로 두면 안 되는 것이었다. 정렬하지 않고 보내면 같은 두 사람이 `dm_A_B`와
`dm_B_A`를 각각 만들 수 있고, 그러면 애초에 결정적 ID를 쓴 이유가 사라진다. 규칙에는 정렬
함수가 없지만 **비교는 있다** — `memberUids[0] < memberUids[1]` 한 줄로 끝난다.

**2. `dm_` 접두사를 가진 ID는 DM에만 내준다.** 안 그러면 아무나 `dm_A_B` 자리에 공개 채널을
하나 만들어 두는 것만으로 A와 B가 영영 DM을 못 열게 막을 수 있다. 그 ID는 이미 차 있고,
두 사람에게는 다른 ID를 쓸 방법이 없다. 규칙 테스트 `[DM] dm_ 자리를 일반 채널로 차지할 수
없다`가 이것이다.

**3. "있는지 확인하고 없으면 만든다"를 읽기로 하지 않는다.** 없는 문서를 `getDoc`으로 찔러
보는 방법은 여기서 통하지 않는다 — 채널 read 규칙이 `resource.data.visibility`를 보는데
문서가 없으면 `resource`가 null이라 평가가 실패하고, "없음"이 아니라 **권한 거부**로 돌아온다.
§1의 "규칙은 필터가 아니다"와 같은 계열의 함정이다.

그럴 필요도 없다. 내가 낀 DM은 전부 사이드바 목록 쿼리(`memberUids array-contains me`)에
이미 들어와 있으므로, 화면이 들고 있는 목록에 없으면 없는 것이다(추가 읽기 0회). 두 사람이
같은 순간에 서로에게 말을 거는 경우만 남는데, 늦은 쪽의 `setDoc`은 상대 문서를 덮는 update가
되어 규칙에 막힌다 — **막히는 것이 맞다.** 덮으면 `lastMessageAt`이 날아가 상대 사이드바의
안읽음 점이 조용히 사라진다. 대화는 이미 생겼으니 그대로 열면 된다(`channelActions.openDm`).

**4. DM은 `lastMessageAt` 말고는 아무도 못 고치고, 아무도 못 지운다.** §10이 관리자를 열람에서
뺐는데 쓰기를 열어 두면 예외가 반쪽이 된다. 참여자 추가를 막는 이유는 따로 있다 — 한 명을 더
넣으면 2인 대화가 아닌데 문서 ID는 여전히 두 사람을 가리켜 ID가 뜻을 잃는다.

**5. `memberNames`를 문서에 박아 둔다(스키마 추가).** 사이드바는 교직원 명단을 읽지 않는다
(채널은 이름이 문서에 있으니 필요가 없었다). DM만을 위해 명단 조회를 사이드바에 들이면 화면
하나 그리려고 `users`를 통째로 읽게 된다. 메시지의 `authorName`과 같은 판단이고, 학교를 떠난
사람과의 지난 대화가 uid로 보이지 않는다는 이점도 같다.
...
## 10.5 P1 구현 기록 (2026-08-24)

배포 순서는 **백필 → 클라이언트·규칙**으로 갔다(§9). 백필은 필드를 더하기만 하므로 옛
규칙·옛 클라이언트와 충돌하지 않아 먼저 돌려도 안전했다.

- 백필 결과: seonyoo-hs 채널 1건·글 4건. 나머지 25개 학교(게스트 포함)는 0건.
  **채널 기능이 아직 실사용 전이라 마이그레이션 위험이 사실상 없었다**
- `visibleUids`는 공개 글에도 빈 배열로 넣었다. 규칙이 `uid in resource.data.visibleUids`를
  평가할 때 필드가 없으면 터지는데, `||` 단축 평가에 기대는 것보다 값이 있는 편이 낫다.
  빈 배열이라 비용도 없다
- 규칙에서 `visibility`·`memberUids`는 `.get(기본값)`으로 감싸지 **않았다.** 감싸면 규칙
  엔진이 `array-contains` 쿼리와 조건을 맞물리지 못해 목록 쿼리가 통째로 거부될 위험이 있다.
  백필을 먼저 한 이유가 이것이다. `type`만 예외로 `.get()`을 썼다 — 아직 `type`으로 거르는
  쿼리가 없어 안전성 증명과 무관하다

### 규칙 테스트가 실제로 잡아낸 버그 (2026-08-24) ★

에뮬레이터 환경(Java 21 + `@firebase/rules-unit-testing`)을 갖추고 테스트를 돌리자마자
**이미 배포한 규칙의 회귀를 잡았다.**

§2에서 "대상자 기반 쿼리는 이미 자기 스코프라 안전하다"고 적었는데 **틀렸다.** 자기 스코프인
것과 새 규칙 조건에 맞물리는 것은 다른 문제다. 규칙을 `visibility`/`visibleUids` 두 갈래로만
두었더니, `useHomeFeed`와 `useDesktopNotifications`가 쓰는

```js
where('targetUids', 'array-contains', uid)
```

쿼리가 어느 조건도 보장하지 못해 **통째로 거부됐다.** 즉 홈 화면과 데스크톱 알림이 조용히
죽은 채로 배포돼 있었다. 채널 화면만 눈으로 확인했기 때문에 못 봤다.

→ read 조건을 네 갈래로 늘렸다. 관대해서가 아니라 **클라이언트가 쓰는 쿼리마다 맞물릴 조건이
하나씩 있어야 하기 때문**이다:

| 조건 | 이 조건이 없으면 죽는 쿼리 |
|---|---|
| `visibility == 'school'` | `useChannels` 공개 글 리스너 |
| `uid in visibleUids` | `useChannels` 비공개 글 리스너 |
| `uid in targetUids` | `useHomeFeed`, `useDesktopNotifications`, `CommandPalette` |
| `createdBy == uid` | `RequestList` 내가 보낸 글, `CommandPalette` |

뒤의 둘은 의미로도 옳다 — 나에게 온 글과 내가 쓴 글을 못 읽으면 그건 기능이 아니다.

**교훈**: 규칙을 고칠 때는 "규칙이 맞는가"만이 아니라 **"클라이언트의 모든 쿼리가 아직
통과하는가"**를 함께 봐야 한다. 후자는 코드를 읽어서는 놓치기 쉽고, 화면으로도 안 잡힌다
(홈이 빈 화면이 될 뿐 오류가 안 뜬다). `tests/firestore.rules.test.js`에 쿼리별 회귀 테스트를
박아두었다.

**검증 환경**: `npm run test:rules` (Java 필요 — winget으로 Microsoft OpenJDK 21 설치함).
기본 `npm test`와 분리했다 — 에뮬레이터가 없는 환경에서 통째로 실패하면 안 되기 때문이다.
...
- [x] ~~§10 관리자 접근 정책 결정~~ — 확정(2026-08-24): 슈퍼 관리자 못 봄 / 학교 관리자는
      업무 채널만, DM 제외
- [x] ~~백필 스크립트 작성 → 실행 → 검증~~ — 완료(2026-08-24). 채널 1건·글 4건
- [x] ~~비공개 채널 유출 경로 점검~~ — 목록·직접 URL·관리자·슈퍼관리자 전부 테스트로 확인.
      검색은 P5에서 만들 때 함께 본다
- [ ] 첨부파일을 메시지 v1에 넣을지 판단(§6) — P2 사안
- [ ] `get()` 횟수 제한 재확인 — P2에서 메시지 규칙에 get()을 쓰게 되면 그때 확인

### 규칙 테스트 — `npm run test:rules` (2026-08-24 구축, 21개 통과)

`firestore.rules` 변경은 화면으로 검증할 수 없다. 막혀야 할 것이 막혔는지는 **안 보이는 것을
확인하는 일**이고, 비멤버 계정이 하나 더 있어야 하는데 실사용 계정이 하나뿐이라 손으로는
아예 불가능하다.
...
| 남의 uid를 넣은 DM 문서 ID로 채널 생성 | 거부(§8) — **P2에서 추가** |

위 표는 전부 `tests/firestore.rules.test.js`에 들어 있고 통과한다(DM 문서 ID 검증만 P2 대기).
여기에 더해 **쿼리별 회귀 테스트**가 함께 있다 — 클라이언트가 쓰는 쿼리가 규칙 변경 뒤에도
통과하는지 보는 것으로, 실제로 홈 화면이 죽은 것을 이걸로 잡았다(§10.5).
