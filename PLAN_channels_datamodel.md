...
**검증 한계 (정직하게)**: 규칙 단위 테스트를 돌리지 못했다. Firestore 에뮬레이터는 Java가
필요한데 이 PC에 없다. 그래서 §11의 규칙 테스트 표는 **여전히 미완**이며, P1은 배포 후
실제 화면 확인으로 대신했다. P2(메시지)에서 규칙이 더 복잡해지므로 그 전에 에뮬레이터
환경을 갖추는 것이 좋다.
