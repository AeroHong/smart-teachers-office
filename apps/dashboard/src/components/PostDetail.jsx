...
export default function PostDetail({ requestId, onDeleted, onOpenBlockComments }) {
  ...
  return (
    <>
      {/* 표지 — CanvasEditor.jsx의 편집 화면과 같은 160px 배너, objectFit:'cover'. 아래
          maxWidth:1120·p:2.5 칸 바깥(Fragment 형제)에 둬서 좌우 꽉 채운다(2026-08-28,
          사용자 요청 — 편집 화면에 이어 읽기 화면도 동일하게). WorkspaceLayout의
          component="main"에는 자체 padding이 없어 별도 마이너스 마진 없이도 그대로
          가장자리까지 닿는다. 읽기 화면이라 바꾸기·삭제는 없다(수정은 '수정' 버튼으로
          편집 화면에서). */}
