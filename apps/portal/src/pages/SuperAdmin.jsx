...
export default function SuperAdmin() {
  ...
  // ── 도메인 저장 (schoolDomains 이동 + schools.domains 업데이트) ─
  const handleSaveDomain = async (schoolId, oldDomain) => {
    const newDomain = domainInput.trim().toLowerCase().replace(/^@/, '')
    if (newDomain === oldDomain) { setEditingDomain(null); return }
    if (newDomain && !/^[a-z0-9.-]+\.[a-z]{2,}$/.test(newDomain)) {
      setError('올바른 도메인 형식을 입력하세요. (예: sunyu.hs.kr)')
      return
    }
    if (newDomain) {
      const existing = await getDoc(doc(db, 'schoolDomains', newDomain))
      if (existing.exists() && existing.data().schoolId !== schoolId) {
        setError(`@${newDomain} 은 이미 다른 학교에 등록된 도메인입니다.`)
        return
      }
    }
    try {
      // 기존 도메인 문서 삭제
      if (oldDomain) await deleteDoc(doc(db, 'schoolDomains', oldDomain))
      // 새 도메인 문서 생성
      if (newDomain) {
        await setDoc(doc(db, 'schoolDomains', newDomain), {
          schoolId,
          createdAt: serverTimestamp(),
          createdBy: user.email,
        })
      ...
      await setDoc(doc(db, 'schools', schoolId), {
        domains: newDomain ? [newDomain] : [],
      }, { merge: true })
      await logAudit(user.email, 'school_domain_changed', { schoolId, from: oldDomain, to: newDomain })
      showSuccess(`✅ 도메인 변경 완료`)
      setEditingDomain(null)
      await loadSchools()
    } catch (e) {
      setError('도메인 변경 실패: ' + e.message)
    }
  }
                    ...
                    {editingDomain === schoolId ? (
                      <Box sx={{ display: 'flex', gap: 0.5, alignItems: 'center' }}>
                        <TextField
                          ...
                          InputProps={{ startAdornment: <span style={{ color: '#aaa', marginRight: 2 }}>@</span> }}
                          onKeyDown={e => {
                            if (e.key === 'Enter') handleSaveDomain(schoolId, domain)
