...
export default function StudentList() {
  ...
  const [groupStudentsMap, setGroupStudentsMap] = useState({}) // groupId -> student[]

  // 그룹 편집
  const [editingGroupId, setEditingGroupId] = useState(null)
  const [editingGroupName, setEditingGroupName] = useState('')
  const [addStudentDraft, setAddStudentDraft] = useState({}) // groupId -> { sid, name }
  const [groupCsvAdding, setGroupCsvAdding] = useState({})  // groupId -> boolean (CSV 추가 중)
  const [historyStudent, setHistoryStudent] = useState(null) // 이력 모달 대상 학생
  ...
  // 학교관리자 전용: 교사 목록 + 담당 교사 선택
  const [teachers, setTeachers] = useState([])
  const [assignedTeacher, setAssignedTeacher] = useState('')  // uid
  ...
  const handleToggleGroup = async (group) => {
    ...
    if (groupStudentsMap[group.id]) return // 이미 로드됨
  ...
  // ── CSV 파싱 (헤더 기반 자동 컬럼 탐색, UTF-8 고정) ────────────
  const parseCSV = (text) => {
...
const styles = {
  ...
  encodingRow:  { display: 'flex', alignItems: 'center', gap: '0.75rem', flexWrap: 'wrap' },
  encodingLabel:{ fontSize: '0.85rem', fontWeight: 600 },
  radioLabel:   { fontSize: '0.85rem', display: 'flex', alignItems: 'center', gap: '0.25rem', cursor: 'pointer' },
  encodingHint: { fontSize: '0.78rem', color: '#999' },
