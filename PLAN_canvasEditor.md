...
- **"작성 중이던 캔버스로 돌아왔을 때 제출현황이 아니라 계속 작성 화면이 나와야
  한다"** ✅ 해결(2026-08-26, 사용자 확정). 겉보기엔 간단해 보였지만, 제출현황
  (완료 N/M명·미완료 명단·다시 알림·마감하기)이 **글쓴이가 자기 요청을 관리하는
  유일한 자리**(PostComposer엔 그 기능이 전혀 없음)라 "무조건 편집기로"만 하면 관리
  기능에 손을 못 대는 회귀가 생겨서 사용자에게 다시 물었다. 답: **마감 여부와 무관하게
  글쓴이는 캔버스를 열면 항상 편집기**, 제출현황은 **편집기 안 새 버튼('업무현황
  N/M')을 눌러야** 보기 화면(PostDetail)으로 간다.
  - `Channels.jsx`에 `canvasUrl(post)` 헬퍼 신설 — `post.createdBy === 내 uid`면
    `/edit`을 붙인다. 탭 클릭·더보기 메뉴·보관된 글 목록 세 곳 모두 이걸로 통일.
    (메시지 카드로 전달받은 캔버스 열기는 이번에 안 건드렸다 — refChannelId만 있고
    글쓴이 정보가 없어 판정하려면 추가 조회가 필요하다. 흔치 않은 경로라 범위 밖.)
  - `PostComposer.jsx`에 `completedUids` 상태 추가(고칠 글 읽어올 때 한 번 채움,
    실시간 아님 — 정확한 값은 버튼을 눌러 들어간 화면이 보여준다) + `요청/안내` 선택
    바로 옆에 `업무현황 {완료}/{전체}` 버튼. 요청(`needsCompletion`)이고 문서가 이미
    있을 때만(`created`) 보인다. 클릭하면 이미 있는 `onOpenCanvasRef` 콜백을 그대로
    재사용해 `/channels/{channelId}/{requestId}`(보기 주소)로 보낸다 — 새 prop 안 만듦.
