...
## v2.3.2 버그수정 (2026-04-16)

### 보강 신청 시스템 — 명예의 전당 수정
- **명예의 전당 데이터 미표시 버그 수정**
  - 원인 1 (`Code.gs`): `getStats`에서 컬럼명을 `이름`으로 탐색했으나 실제 헤더는 `교사명` → `indexOf` 결과 -1로 전원 제외됨
  - 원인 2 (`CoverStatus.jsx`): 월별 탭에서 `monthCount > 0` 필터로 인해 이번 달 보강 횟수 없는 선생님 전원 제외됨
  - 수정: `Code.gs` 컬럼명 `교사명`으로 통일, 식별 기준을 이름 → 이메일로 변경
  - 수정: 월별 탭 필터 제거 → 전 교사 `monthCount` 기준 정렬 표시

---

## v2.3.1 버그수정 (2026-04-15)

### 버그수정
- **반복 이벤트 라이브 세션**: 전날 QR 마감 후 다음 날에도 "QR 체크인 마감" 상태로 굳어 출석 시작이 불가한 문제 수정
  - 원인: `processSessionClose` Cloud Function이 수업 종료 시점에 실행되지 못하면 `liveOpenedAt` 등 세션 필드가 전날 값으로 잔존
  - 프론트엔드: 대시보드 진입 시 `liveOpenedAt`이 오늘 날짜가 아닌 경우 세션 필드 자동 초기화
  - 백엔드: `autoManageLiveSessions`에서 전날 세션 감지 시 즉시 강제 초기화 로직 추가
