...
- `apps/desktop/package.json` 버전 0.1.5 → 0.1.6 (main.js/preload.js 변경 — 재설치 필요한
  "껍데기" 변경)
- **빌드 해결 (2026-08-21 저녁, 집 네트워크)**: `npm run build:desktop` 통과, `0.1.7` 인스톨러
  생성. 다만 **당시 세운 가설("`winCodeSign` 캐시를 채워야 한다")은 맞지 않았다** — 빌드 후에도
  ...
  `winCodeSign`을 받으러 가지도 않았다. 학교에서의 실패가 간헐적 차단이었는지 `node_modules`
  상태 차이였는지는 미확정 — **"이제 학교에서도 빌드된다"고 단정할 수 없다**
...
- [x] **웹 갱신 감지** (2026-08-24) — 구현·배포 완료. 아래 "웹 갱신 감지 구현 상세" 참고
...
- [x] **껍데기 자동 업데이트(`electron-updater`) 구조** (2026-08-21) — 구현·실동작 검증 완료
      (v0.1.6→v0.1.7 종단). 아래 상세 참고. 남은 것은 **학교 네트워크에서의 도달 여부** 하나로,
      집 네트워크에서는 `latest.yml` 조회까지 정상 동작을 로그로 확인했다.
      코드 서명은 여전히 미정 — 서명 없이도 설치·업데이트 자체는 되지만, 매번 SmartScreen
      경고를 볼 수 있다(전 교직원 배포 전 결정 필요, "다른 PC 검증" 항목과 연결됨)

### 웹 갱신 감지 구현 상세 (2026-08-24)

**별도 버전 파일을 만들지 않았다.** Vite가 만드는 번들 이름에 이미 내용 해시가 들어 있어
(`index-c1Ui3EDA.js`), `index.html`이 가리키는 스크립트 이름만 견주면 새 배포인지 알 수 있다.
버전을 손으로 올리는 방식이면 올리는 것을 잊는 순간 감지가 조용히 죽는다.

`firebase.json`에서 `index.html`은 `no-cache, max-age=0`, `/assets/**`는 `immutable`이라
이 방식이 성립한다. **캐시 정책을 바꾸면 이 기능도 함께 봐야 한다.**

- `apps/shared/lib/appVersion.js` — 순수 판정. 스크립트 주소들에서 `/assets/*.js`만 골라
  정렬해 이은 "배포 서명"을 만든다. 조각을 전부 모으는 이유는 코드 분할이 들어왔을 때
  진입 번들 이름은 그대로인데 다른 조각만 바뀌는 경우를 놓치지 않기 위해서다
- `apps/dashboard/src/lib/useAppUpdate.js` — 10분 간격 + **창이 다시 앞으로 나올 때**
  확인. 뒤의 것이 더 중요하다. 종일 켜두는 앱에서 사람이 실제로 화면을 보는 순간이 그때고,
  마침 그때가 새로고침을 눌러도 괜찮은 순간이기도 하다(다른 일을 하다 돌아온 참이라
  쓰던 글을 잃을 걱정이 적다)
- `apps/dashboard/src/components/UpdateBanner.jsx` — 상단바 **위**에 자리를 차지하는 띠.
  떠 있는 알림은 내용을 가리고, 가리는 알림은 읽기 전에 닫게 된다
- **자동 새로고침은 하지 않는다.** 글을 쓰던 중이면 쓰던 내용이 사라지고, 한 번 겪으면
  이 앱에서 긴 글을 안 쓰게 된다
- 닫기는 그 배포에 대해서만. 다음 배포가 올라오면 서명이 달라져 다시 뜬다 — 한 번 닫았다고
  영영 조용해지면 "종일 옛 코드로 돈다"는 원래 문제가 그대로 남는다

**헛띄움을 막는 것이 이 기능의 핵심**이다. 아무 일도 없는데 띠가 한 번 뜨면 그 다음부터는
진짜 배포가 있어도 아무도 안 누른다. 그래서 서명을 하나라도 못 읽으면(오프라인, 학교 망이
응답을 가로채 로그인 페이지를 돌려주는 경우 — 빌드 도구 다운로드가 가로채진 전례가 있다)
**갱신이 아니라고 답한다.** 개발 서버에는 해시 번들이 없어 서명이 비고, 그래서 판정 자체가
일어나지 않는다. 테스트 9개가 이 경우들을 지킨다.

**한계 — 이번 배포부터 곧바로 듣지는 않는다.** 지금 앱을 켜두고 있는 사람은 이 감지 코드가
없는 옛 번들을 들고 있다. 한 번은 손으로 새로고침해야 하고, **그 뒤의 배포부터** 띠가 뜬다.
